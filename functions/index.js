...
exports.onCreateFollower = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    // Adding status
     const followedUserStoriesRef = admin
      .firestore()
      .collection("posts")
      .doc(userId)
    ...
    const querySnapshotStories = await followedUserStoriesRef.get();
    ...
    querySnapshotStories.forEach(doc => {
      if (doc.exists) {
       const storyId = doc.id;
       const storyData = doc.data();
       timelineStoriesRef.doc(userId).set(storyData);
      }
    });
  });
...
exports.onDeleteFollower = functions.firestore
  ...
  .onDelete(async (snapshot, context) => {
    ...
    const querySnapshotStories = await timelineStoriesRef.get();
    querySnapshotStories.forEach(doc => {
      if (doc.exists) {
        doc.ref.delete();
      }
...
// User status is created, add status to timeline of each follower (of post owner)

exports.onCreateStory = functions.firestore
  .document("/posts/{userId}/userStories/{storyId}")
  .onCreate(async (snapshot, context) => {
...
//  .document("/posts/{userId}/userPosts/{storyId}")
//  .onDelete(async (snapshot, context) => {
//    const userId = context.params.userId;
//    const storyId = context.params.storyId;
//
